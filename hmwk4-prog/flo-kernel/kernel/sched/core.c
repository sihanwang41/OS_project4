...
need_resched:
	...
	if (unlikely(!rq->grr.grr_nr_running))
		idle_balance_grr(cpu, rq);
...
void do_set_cpus_allowed(struct task_struct *p, const struct cpumask *new_mask)
{
	...
	p->grr.nr_cpus_allowed = cpumask_weight(new_mask);
}
...
void __init sched_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		set_grr_rq_group(&rq->grr, rq->cpu);
			      ...
			      struct cgroup_taskset *tset)
{
	...
	cgroup_taskset_for_each(task, cgrp, tset) {
		...
		if (task->sched_class->cgroup_attach)
			task->sched_class->cgroup_attach(task);
	}
}
...
/*
 * System call for changing the groups assigned to CPUs
 * for the GRR scheduler
 */
SYSCALL_DEFINE2(sched_set_CPUgroup, int, numCPU, int, group)
{
	...
	if (numCPU < 1 || numCPU > num_possible_cpus() - 1)
		return -EINVAL;

	if (!(group == FOREGROUND || group == BACKGROUND))
		return -EINVAL;

	n = group == FOREGROUND ? numCPU : num_possible_cpus() - numCPU;
	change_grr_rq_groups(n);
