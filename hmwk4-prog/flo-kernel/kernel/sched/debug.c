...
void print_grr_rq(struct seq_file *m, int cpu, struct grr_rq *grr_rq)
{
	...
	SEQ_printf(m, "\ngrr_rq: ");
	list_for_each_entry_safe(grr_se, grr_se_temp, &grr_rq->run_list, run_list) {
		p = container_of(grr_se, struct task_struct, grr);
